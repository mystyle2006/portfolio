...
export const NavOrb = ({
  ...
  return (
    <div
      className="w-[140px] h-[140px] rounded-full flex flex-col items-center justify-center cursor-pointer select-none
        shadow-[0_4px_20px_rgba(0,0,0,0.2)] hover:scale-105
        transition-[box-shadow,transform] duration-200 gap-1"
      style={{
        background: bg,
        opacity: visible ? 1 : 0,
        animation,
    ...
    >
      <span className="text-[14px] text-zinc-700 font-bold tracking-wide text-center leading-tight px-4">
        {title}
      </span>
      {subtitle && (
        <span className="text-[10px] text-zinc-500 font-medium text-center leading-snug px-4">
          {subtitle}
        </span>
      )}
    </div>
